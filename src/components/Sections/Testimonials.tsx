...
const Testimonial: FC<{testimonial: TestimonialType; isActive: boolean}> = memo(
  ({testimonial: {text, name, image, title}, isActive}) => (
    <div
      className={classNames(
      ...
      {image ? (
        <div className="relative h-14 w-14 shrink-0 sm:h-16 sm:w-16">
          ...
          <Image className="h-full w-full rounded-full" alt={name} src={image} />
        </div>
      ) : (
        <QuoteIcon className="h-5 w-5 shrink-0 text-white sm:h-8 sm:w-8" />
      )}
      <div className="flex flex-col gap-y-2">
        <p className="prose prose-sm font-medium italic text-white sm:prose-base mb-4">{text}</p>
        <p className="text-xs italic text-white sm:text-sm md:text-base lg:text-lg">{name},</p>
        <p className="text-xs italic text-white sm:text-sm md:text-base lg:text-lg">{title}</p>
      </div>
    </div>
  ),
);
